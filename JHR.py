...
# Camille Payant 2019
# coding: utf-8
l1 = ["La Presse+, ACTUALITÉS,  samedi 26 janvier 2019 618 mots, p. ACTUALITÉS_12",
...
"Le Soleil, Sports,  samedi 26 janvier 2019 299 mots, p. 57", ### Cette ligne se trouve à l'extérieur de ta liste l1.
...
for article in l1:
	...
	section = article[1] ### Il manquait un petit «.strip()» ici, comme dans tes deux variables suivantes
	section = section.strip()

	date = article[2]

	mots = date.replace('  samedi 26 janvier 2019','') ### Bonne stratégie! :)
	mots = mots.strip() ### Pour retrancher espaces blancs superflus

	pages = article[3:] ### Ici, il ne fallait pas inclure «:» entre les crochets. Cela fait en sorte que ton résultat est de type liste (une liste qui ne contient qu'un seul élément).
	### «article[3]» donne comme résultat le 4e élément et l'affiche en fonction de son type d'origine (ici, du texte)
	
	### Je crée une nouvelle variable «pageJH» pour montrer la différence:
	pageJH = article[3].strip()

### Dans ton affichage, il manque quelques espaces. Je le mets en commentaire et t'en propose un autre:
	# print ("Le quotidien " + str(media) + "a publié dans " + str(section) + "aux " + str(pages) + "un article contenant" + str(mots))
	print("{} publie, dans sa section {}, un article de {} en {}".format(media,section,mots,pageJH))
...
### Bonne idée pour la suite! Mais...
### ... cette seconde boucle aurait pu être incluse dans la première
...
for article in l1:
	...
	if media == 'La Voix de': ### Ici, le nom du média n'est pas complet, donc, le total donne 0...
		lavoix += 1

	if media == 'La Tribune':
		latribune += 1

	if media == 'Le Nouvelliste':
		lenouvelliste += 1


print("Le quotidien Le Soleil a publié " + str(lesoleil) + "articles.")

print("Le quotidien La Presse+ a publié " + str(lapresse) + "articles.")

print("Le quotidien Le Devoir a publié " + str(ledevoir) + "articles.")

print("Le quotidien La Voix de l'Est a publié " + str(lavoix) + "articles.")

print("Le quotidien La Tribune a publié " + str(latribune) + "articles.")

print("Le quotidien Le Nouvelliste a publié " + str(lenouvelliste) + "articles.")